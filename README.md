...
Puuduv või aegunud OHLCV-ajalugu uuendatakse automaatselt 120 päeva ulatuses serveri käivitamisel, igal ajastatud turuandmete värskendusel (vaikimisi iga 3 tunni järel, `FUNNEL_INTERVAL_HOURS`) ja käsitsi käivitatud värskendusel. Kontroll hõlmab kõiki aktiivseid instrumente, kõiki avatud positsioone ja SPY-d, mitte ainult päeva sõelakandidaate. Enne iga otsustuspartii tehniliste näitajate arvutamist tehakse täiendav kontroll ja vajadusel uus päring. Värskust kontrollitakse viimase lõppenud NYSE sessiooni järgi, arvestades pühi ja varajasi sulgemisi; poolelioleva päeva küünlaid ei kasutata. Kui uuendamine ebaõnnestub, märgitakse näitajad puuduvaks või aegunuks, mitte ei esitata vana ajalugu värske momentumi, volatiilsuse või mahukinnitusena. SPY suhteline tootlus kasutab sama hetktõmmise hinda. Ajaloo uuendamine ei käivita AI otsuseid ega tehinguid. Yahoo võib tagastada sessioonile kuupäeva, kuid `null` OHLCV-väärtused; selline rida ei ole kasutatav turuandmestik. Viimase nädala puuduvaid sessioone proovitakse samas tsüklis veel kord lühema, 10-päevase päringuaknaga, sest Yahoo võib eri päringuakendes anda erineva tulemuse. Allesjäänud lüngad proovitakse järgmisel värskendusel uuesti alla laadida, mitte ei täideta väljamõeldud hindade ega tunniandmetest tuletatud päevamahuga.

Igas veebivaates on nähtav **Historical data health** paneel: tehniliste näitajate jaoks kasutatavate instrumentide arv, viimase kontrolli aeg, nõutud sessioon, järgmine värskendus ning probleemsete sümbolite ja puuduvate sessioonide loend. **Refresh data now** käivitab ajaloo, hindade ja uudiste värskenduse ilma investeerimisotsusteta. Ka staatuse päringu või ajaloo värskenduse viga kuvatakse hoiatusena; edukas uudiste päring ei peida halba ajalookvaliteeti. `GET /api/cycle/status` väljad `history` ja `history_in_progress` annavad sama info masinloetavalt.
...
Ühe mudeli strateegiakontode pakkujad on DeepSeek (vaikimisi), Groq ja kohalik Ollama. Enne andmebaasi loomist valitakse `LLM_PROVIDER`-i ja vastava mudelimuutujaga neile üks ühine mudel. Komiteekonto kasutab eraldi pi protsessi kaudu GitHub Copiloti OAuth-autentimist: vaikimisi on nõustajad `claude-opus-4.8`, `gpt-5.6-terra` ja `kimi-k3` ning eesistuja `gpt-5.6-sol`. Kõik mudelisidumised, mudelipõhised vastused, räsidega sisendid ja lõplik otsus talletatakse auditiks. Iga välise pi-kõne juures talletatakse ka pi seansi ID, täielik tokenikasutuse JSON ja pi mudelikataloogi hinnal põhinev hinnanguline USD-kulu; komitee kogukulu on kõigi selle partii mudelisammude summa, sealhulgas võimaliku riskivarumudeli kõne. Tellimuspõhise GitHub Copiloti puhul on see võrreldav hinnang, mitte tingimata tegelik arvesumma. Ükski mudel ei kirjuta andmebaasi ega kasuta pi faili-, shelli- või muid tööriistu.

Komitee vajab vähemalt kahte kehtivat nõustajaettepanekut, millest üks peab olema sõltumatu riskihinnang. Riskinõustaja vea või vigase vastuse korral proovib süsteem eraldi varumudelit (`PI_COPILOT_RISK_FALLBACK_MODEL`, vaikimisi `gpt-5.4`); ka varumudeli ebaõnnestumisel eesistujat ei käivitata ega tehinguid lubata. Ebaõnnestunud ja asendavad mudelisammud jäävad auditisse ning kuvatakse viimase partii juures. Komitee müük-ost rotatsioonis ei proovita ostu, kui eelnev müük ebaõnnestus. Need on töökindluse parandused, mitte tootluse garantii; autonoomse konto investeerimispiirangud ei muutu.

### Uurimismetoodika

Võrdlus sisaldab seitset sama mudeliga strateegiakontot, üht mitme mudeli komiteekontot, inimkontot ja passiivset indeksikontot. Kõik saavad sama partii muutumatu turuhetktõmmise ja alluvad samale täitmismootorile, kuid komitee kasutab ühe otsuse jaoks tavaliselt nelja mudelisammu ning vajadusel täiendavat riskivarumudelit ja korduskatseid. Tulemuste tõlgendamisel käsitletakse komiteed eraldi **AI Ensemble** rühmana, sest selle mudelivalik ja arvutusressurss erinevad ühe mudeli kontodest. Seetõttu ei saa komitee paremat või halvemat tootlust omistada ainult investeerimisstrateegiale.
...
| `PI_COPILOT_RISK_FALLBACK_MODEL` | `gpt-5.4` | riskinõustaja varumudel; peab kõigist nõustajatest ja eesistujast erinema; tühi väärtus keelab varukõne |
