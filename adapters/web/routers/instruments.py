...
async def add_instrument(request: Request, data: InstrumentRequest, _: OperatorAccess):
    try:
...
async def change_instrument_active(
    ticker: str,
    request: Request,
    data: InstrumentActivationRequest,
    _: OperatorAccess,
):
    try:
...
async def import_etfs(request: Request, _: OperatorAccess, dry_run: bool = False):
